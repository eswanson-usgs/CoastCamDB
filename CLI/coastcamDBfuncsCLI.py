    ...
    try:
        ...
        for i, name in enumerate(shortName):
            ...
            if str(name) in filename:
                ...
                filenameEl = filename.split('.')
                unixTime = filenameEl[0]
...
def storeReadData(dataframe, scope, table, csvPath='', dataDict = {}):
    '''
    ...
    if scope == 'column':
        ...
        for value in dataframe.get(columnName):
            columnValues.append(value)

        dataDict[columnName] = columnValues

        if csvPath != '':
            filename = table + '_' + columnName + '.csv'
        ...
        if csvPath != '':
            ...
            filename = table + '.csv'
            folderPath = csvPath + '/sites/' + site + '/tables/'
    ...
    def insertTable2db(self):
        ...
        if (self.tableName == 'geometry') or (self.tableName == 'usedgcp'):
            for i in range(0, len(otherColumns)):
                otherColumns[i].insert2db(fkArgs=fkArgs)
        else:
            for i in range(0, len(otherColumns)):
                otherColumns[i].insert2db(fkArgs=fkArgs)
...
class Column:
    ...
    def inputIDSeq(self, value):
        ...
        if (self.table.tableName == 'usedgcp') or (self.table.tableName == 'geometry'):
            key = "seq"
            #account for NULL values
            if value == 'None':
                query = "SELECT seq, {} FROM {} WHERE {} IS NULL".format(self.columnName, self.table.tableName, self.columnName)
            else:
                query = "SELECT seq, {} FROM {} WHERE {} = '{}'".format(self.columnName, self.table.tableName, self.columnName, value)
        else:
            key = "id"
            if value == 'None':
                query = "SELECT id, {} FROM {} WHERE {} IS NULL".format(self.columnName, self.table.tableName, self.columnName)
            else:
                query = "SELECT id, {} FROM {} WHERE {} = '{}'".format(self.columnName, self.table.tableName, self.columnName, value)
        ...
        for identifier in result.get(key):
            availIDSeq.append(str(identifier))
    ...
    def getFKArgs(self):
        ...
        if (self.table.tableName == 'station') or (self.table.tableName == 'gcp'):
            key = 'siteID'

            try:
                #if empty foreign key columns, throw error
                if self.table.tableName == 'station':
                ...
                result = pd.read_sql(query, con=self.connection)
                if result.size == 0:
                    raise FKError(message="no foreign key values in table '{}'".format(self.table.tableName))
            except FKError as e:
                sys.exit(e.message)
            
        elif self.table.tableName == 'camera':
            key = 'stationID'
        ...
        elif self.table.tableName == 'geometry':
            key = 'cameraID'
        ...
        query = "SELECT {} FROM {} ".format(key, self.table.tableName)
        result = getFormattedResult(query, self.connection)
        result = result.drop_duplicates(subset=key, keep='first')
        resultStr = result.to_string(header=False)
        print("Please enter a {} listed from the values below.".format(key))
        print("vv  Available options are listed below vv")
        print(resultStr)
        ...
        while not isGoodID:
            ...
            fkValue = input("~~~Enter a {}: ".format(key))

            if fkValue.strip() == 'quit':
                quit()
                
            if fkValue.strip() in availID:
                isGoodID = True
                fkArgs.append({key : fkValue})
            else:
                print('Invalid {}, please try again.\n'.format(key))

        query = "SELECT {} FROM {} WHERE {} = '{}'".format(self.columnName, self.table.tableName, key, fkValue)
        result = getFormattedResult(query, self.connection)
        resultStr = result.to_string(header=False)
        print("\nCurrent value(s) in {} for {} where {} = {}".format(self.table.tableName, self.columnName, key, fkValue))
        print(resultStr)
    ...
    def update2db(self, oldValue, idList=[], seqList=[], fkArgs=[], returnSeqListFlag=False):
        '''
...
##        if (self.table.tableName == 'camera') or (self.table.tableName == 'usedgcp'):
