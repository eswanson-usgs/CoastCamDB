...
def createYAMLfiles(stationID, outputPath, connection):
    ...
    metadataDescriptorDict['camera_number'] = 'camera number for the corresponding station'
    ...
    for i in range(0, len(cameraList)):
        ...
        #extrinsics
        fileName = shortName.replace(' ', '_') + '_C' + str(camera_number) + '_extr'
        DBdict2yaml(extrinsics[i], extrinsicDescriptorDict, outputPath, fileName)

        #intrinsics
        fileName = shortName.replace(' ', '_') + '_C' + str(camera_number) + '_intr'
        DBdict2yaml(intrinsics[i], intrinsicDescriptorDict, outputPath, fileName)

        #metadata
        fileName = shortName.replace(' ', '_') + '_C' + str(camera_number) + '_metadata'
